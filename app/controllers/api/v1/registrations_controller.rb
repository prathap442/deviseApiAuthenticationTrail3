  ...
  def create
    ...
    if resource.persisted? # the persisted method checks if the records exists in the data base 
      if resource.active_for_authentication? # it checks whether the resource is active for the authentication to be done
        ...
        render json: {resource: resource, location: after_sign_up_path_for(resource), notice: 'Succesfully Registered User' },status: 201
      ...
      render json: {resource: resource,msg: 'resource has got some validations and hence it is inactive for activation',errors: "#{resource.errors.messages}"},status: 404
    end
  end
